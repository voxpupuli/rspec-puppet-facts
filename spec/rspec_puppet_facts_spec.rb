...
describe RspecPuppetFacts do
  ...
  describe '#on_supported_operatingsystem' do

    context 'Without specifying supported_os' do
      subject { on_supported_operatingsystem }
      ...
      context 'With a metadata.json' do
        ...
        context 'With a valid metadata.json' do
          ...
          it 'should return supported OS' do
            expect(subject.map{|h| "#{h[:operatingsystem]}-#{h[:operatingsystemmajrelease]}"}).to eq %w(
              Debian-6
              Debian-7
              RedHat-5
              RedHat-6
